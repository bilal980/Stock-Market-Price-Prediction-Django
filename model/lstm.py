...
class lstm():
    ...
    def fit_lstm(self,units,batch_size=32, epochs=100, figsize=(20, 13)):
        # lstm
        model = models.Sequential()
        model.add(layers.LSTM(
            input_shape=self.X.shape[1:], units=units, activation='tanh', return_sequences=False))
        model.add(layers.Dense(1))
        model.compile(optimizer='rmsprop', loss='mean_squared_error')

        # fit
        training = model.fit(x=self.X, y=self.y, batch_size=batch_size,
                             epochs=epochs, shuffle=True, verbose=0)
        self.model = training.model

    def predict_lstm(self, ts,ahead, figsize=(20, 13)):
        # preprocess
        ...
        for i in range(ahead):
            ...
            # X = np.array(lst_X)
            X = np.asarray(lst_X).astype(np.float32)
            X = np.reshape(X, (1, 1, X.shape[0]))
        ...
        # plot
        dtf = pd.DataFrame(
            {'actual': ts, 'fitted': lst_fitted, 'pred': np.nan})
        dtf = pd.concat([dtf, pd.DataFrame([dic for dic in lst_preds])])
        self.dtf = dtf.reset_index(drop=True)
        ax = self.dtf.plot(title="pred "+str(ahead)+" ahead",
                           figsize=figsize, linewidth=3)
        ax.grid(True)
        plot = dtf.plot()
        fig = ax.get_figure()
        from django.conf import settings
        fig.savefig(settings.STATICFILES_DIRS[0]+''+'\\csv\\'+'temp.png')
